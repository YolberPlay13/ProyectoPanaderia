...
<div class="row">
    <div class="col-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <form method="post" action="<?= base_url('IngresosControl/guardar') ?>" id="formIngreso">
                            ...
                            <select name="proveedor" id="proveedor" class="form-select" required>
                                ...
                                <?php foreach($proveedores as $prov): ?>
                                    <option value="<?= $prov->id_proveedor ?>">
                                        <?= $prov->nombre_proveedor ?> (NIT: <?= $prov->nit_proveedor ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" id="precio" class="form-control" placeholder="0.00" step="0.01" min="0">
                            </div>
                        </div>
...
function agregarFila() {
    ...
    let productoText = productoSelect.selectedOptions[0].text;
    ...
    nuevaFila.innerHTML = `
        ...
        <td class="text-center">
            <button type="button" class="btn btn-outline-danger btn-sm" onclick="eliminarFila(this)" title="Eliminar producto">
...
function eliminarFila(btn) {
    ...
    // Si no hay filas, mostrar mensaje vacío y deshabilitar botón
    if(contadorFilas === 0) {
...
function actualizarResumen() {
    ...
    filas.forEach(fila => {
        ...
        let precioInput = fila.querySelector('input[name="precio[]"]');
...
document.getElementById('formIngreso').addEventListener('submit', function(e) {
    ...
    if(contadorFilas === 0) {
        e.preventDefault();
        alert('Debe agregar al menos un producto al ingreso');
        return;
    }
    
    // Confirmar el ingreso
    if(!confirm('¿Está seguro de registrar este ingreso? Esta acción actualizará el inventario.')) {
        e.preventDefault();
        return;
    }
});
...
document.addEventListener('keypress', function(e) {
    if(e.key === 'Enter' && (e.target.id === 'cantidad' || e.target.id === 'precio')) {
        e.preventDefault();
        agregarFila();
    }
});

// Auto-focus en cantidad después de seleccionar producto
...
// Auto-focus en precio después de ingresar cantidad
